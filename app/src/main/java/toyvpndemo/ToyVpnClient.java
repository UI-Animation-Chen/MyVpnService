...
public class ToyVpnClient extends Activity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    packages.setText(String.join(", ", prefs.getStringSet(
      Prefs.PACKAGES, new HashSet<String>())));

    findViewById(R.id.connect).setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        if (!ToyVpnClient.this.checkProxyConfigs(proxyHost.getText().toString(),
...
//        if (!ToyVpnClient.this.checkPackages(packageSet)) {
//          return;
//        }
        int serverPortNum;
        try {
        ...
        if (intent != null) {
          ToyVpnClient.this.startActivityForResult(intent, 0);
        } else {
          ToyVpnClient.this.onActivityResult(0, RESULT_OK, null);
        }
      }
    });
  ...
  private boolean checkPackages(Set<String> packageNames) {
    final boolean hasCorrectPackageNames = true;
